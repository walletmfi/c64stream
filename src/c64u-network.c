				...
				while (adapter) {
					...
					if (adapter->Type != MIB_IF_TYPE_LOOPBACK &&
						...
						strncpy(ip_buffer, adapter->IpAddressList.IpAddress.String,
							buffer_size - 1);
						ip_buffer[buffer_size - 1] = '\0';
						obs_log(LOG_INFO, "[C64U] Detected Windows IP address: %s (adapter: %s)",
...
socket_t create_udp_socket(uint32_t port)
{
	...
	if (bind(sock, (struct sockaddr *)&addr, sizeof(addr)) < 0) {
		int error = c64u_get_socket_error();
		obs_log(LOG_ERROR, "[C64U] Failed to bind UDP socket to port %u: %s", port, c64u_get_socket_error_string(error));
		close(sock);
		return INVALID_SOCKET_VALUE;
	}
