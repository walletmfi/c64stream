...
# Test 3: Full plugin linking attempt
...
if [ ${#OBJECT_FILES[@]} -gt 0 ]; then
    ...
    if x86_64-w64-mingw32-gcc "${WINDOWS_CFLAGS[@]}" $WINDOWS_LIBS -shared -o c64u-plugin-for-obs.dll "${OBJECT_FILES[@]}" 2>/dev/null; then
        ...
        echo "    ✅ But individual source files compiled successfully - this is the key test"
    fi
else
...
echo "  ✅ Plugin compilation succeeds (linking may need OBS runtime)"
echo ""
